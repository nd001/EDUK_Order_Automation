...
def login_to_stream_if_required(page):
    """
    Log into Stream only when the login form is present.
    ...
    if (
        user_field.count() == 0
        or not user_field.first.is_visible()
    ):
        print("✓ Stream session is already authenticated.")
        return

    username = os.getenv("STREAM_USERNAME", "").strip()
    password = os.getenv("STREAM_PASSWORD", "")

    if not username or not password:
        raise RuntimeError(
            "Stream login page detected, but STREAM_USERNAME "
            "and/or STREAM_PASSWORD are missing from .env."
        )

    print("Logging into Stream...")

    user_field.fill(username)
    password_field.fill(password)
    login_button.click()

    # Wait for the login form to disappear or for Stream to move on.
    try:
        user_field.wait_for(
            state="hidden",
            timeout=15000,
        ...
        # If the form remains, report any visible Stream login error.
        errors = page.locator(
            "#userIdErrors:visible, #passwordErrors:visible"
        )
        ...
        if error_text:
            raise RuntimeError(
                f"Stream login failed: {error_text}"
            )

        raise RuntimeError(
            "Stream login did not complete within 15 seconds."
        )

    print("✓ Stream login successful.")
